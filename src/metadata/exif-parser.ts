 ...
 * Read-only: extracts and parses EXIF metadata. Does not inject/modify images.
 */
...
function readIFD(reader: DataReader, ifdOffset: number, tiffStart: number): ExifData {
  ...
  if (ifdOffset + 2 > reader.length) return result;

  const entryCount = reader.getUint16(ifdOffset);
  let offset = ifdOffset + 2;

  for (let i = 0; i < entryCount; i++) {
    if (offset + 12 > reader.length) break;
 ...
 * Searches for the Exif\0\0 marker and extracts a bounded chunk for parsing.
 */
